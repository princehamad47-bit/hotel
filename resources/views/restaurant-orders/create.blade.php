...
<div class="card">
    ...
    <form method="POST" action="{{ route('restaurant-orders.store') }}">
        ...
        <div class="grid-2">
            <div class="form-group">
                ...
                <select name="order_type" id="order_type" required>
                    <option value="walk_in" @selected(old('order_type', $selectedReservationId ? 'in_house' : 'walk_in' )=='walk_in' )>Walk In</option>
                    <option value="in_house" @selected(old('order_type', $selectedReservationId ? 'in_house' : 'walk_in' )=='in_house' )>In House</option>
                    <option value="room_service" @selected(old('order_type')=='room_service' )>Room Service</option>
            ...
            <div class="form-group">
                ...
                <select name="status" required>
                    ...
                    <option value="preparing" @selected(old('status')=='preparing' )>Preparing</option>
                    <option value="served" @selected(old('status')=='served' )>Served</option>
        ...
        <div id="reservation-box" style="{{ in_array(old('order_type', $selectedReservationId ? 'in_house' : 'walk_in'), ['in_house', 'room_service']) ? '' : 'display:none;' }}">
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <select name="reservation_id" id="reservation_id">
                        ...
                        <option value="{{ $reservation->id }}"
                            ...
                            data-guest-name="{{ $reservation->guest ? $reservation->guest->first_name . ' ' . $reservation->guest->last_name : '' }}"
        ...
        <div id="walkin-box" style="{{ old('order_type', $selectedReservationId ? 'in_house' : 'walk_in') === 'walk_in' ? '' : 'display:none;' }}">
            <div class="grid-2">
                <div class="form-group">
        ...
        <div class="card" style="margin-top: 10px;">
            ...
            <div id="order-items-wrapper">
                @php
                $oldItems = old('items', [['menu_item_id' => '', 'quantity' => 1, 'notes' => '']]);
                @endphp
